...
namespace jobject
{
    namespace detail
    {
        ...
        {
            static const auto nt_handle = _HASH(GetModuleHandleA).get()(xorstr_("ntdll.dll"));
            return reinterpret_cast<T>(_HASH(GetProcAddress).get()(nt_handle, func));
        }
    }
    JOBJECT_FORCEINLINE const auto init()
    {
        /* set apis */
        const static auto NtCreateJobObject = detail::get<detail::typedefs::NtCreateJobObject_t>(xorstr_("NtCreateJobObject"));
        const static auto NtAssignProcessToJobObject = detail::get<detail::typedefs::NtAssignProcessToJobObject_t>(xorstr_("NtAssignProcessToJobObject"));
        const static auto NtSetInformationJobObject = detail::get<detail::typedefs::NtSetInformationJobObject_t>(xorstr_("NtSetInformationJobObject"));
        ...
        NtAssignProcessToJobObject(job, _HASH(GetCurrentProcess).get()());
